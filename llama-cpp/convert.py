...
class VocabLoader:
    def __init__(self, params: Params, fname_tokenizer: Path) -> None:
        ...
        try:
            self.tokenizer = LlamaTokenizer.from_pretrained(str(fname_tokenizer), trust_remote_code=True)
        except ValueError:
            self.tokenizer = LlamaTokenizer.from_pretrained(str(fname_tokenizer), use_fast=False, trust_remote_code=True)
...
class OutputFile:
    ...
    def write_all(
        ...
        for name, lazy_tensor in model.items():
            ...
            #     lazy_tensor.shape=[1024,2048,3,1]
            # if("conv2.weight" in name):
            #     lazy_tensor.shape=[2048,1024,3,1]
        ...
        for i, ((name, lazy_tensor), ndarray) in enumerate(zip(model.items(), ndarrays)):
            ...
            print(f"[{i+1:{padi}d}/{len(model)}] Writing tensor {name:38s} | size {size:16} | ndarray-size {str(ndarray.shape):16s}  | type {lazy_tensor.data_type.name:4} | T+{int(elapsed):4}")
...
def main(args_in: list[str] | None = None) -> None:
    ...
    # if True:
        ...
        # vocab_dic=vocab.tokenizer.get_vocab()
        # vocab_dic_s=sorted(vocab_dic.items(),key=lambda x:x[1],reverse=False)
        # with open("vocab.txt","w",encoding="utf-8") as fw:
        #     for w,id in vocab_dic_s:
        #         fw.writelines(str(id)+"\t"+w+"\n")
        # exit()
    ...
    ftype   = pick_output_type(model, args.outtype) # 确认要转换的数据类型
    model   = convert_to_output_type(model, ftype) # 转换数据类型
