    ...
    ~llama_kv_cache() {
        ...
        if (ggml_cublas_loaded()) {
            for (size_t i = 0; i < k_l.size(); ++i) {
                ...
                ggml_cuda_free_data(conv_inputs[i]);
            }
        }
#endif
        if (ctx) {
...
static bool llama_kv_cache_init(
    ...
    params.mem_size   = 3u*n_layer*ggml_tensor_overhead();
    ...
    for (int i = 0; i < (int) n_layer; i++) {
        ...
        ggml_tensor * conv_input = ggml_new_tensor_1d(cache.ctx, GGML_TYPE_F32, n_embd* n_ctx);//保存最近的2个
            ...
            case LLM_ARCH_REFACT:
                {
                    ...
                    for (uint32_t i = 0; i < n_layer; ++i) {
                        ...
                        layer.conv1_w = ml.create_tensor(ctx, tn(LLM_TENSOR_CONV1, "weight", i), {1,2,n_embd,n_conv},     backend_split);
                        layer.conv1_b = ml.create_tensor(ctx, tn(LLM_TENSOR_CONV1, "bias", i), {n_conv}, backend, false);
                        layer.conv2_w = ml.create_tensor(ctx, tn(LLM_TENSOR_CONV2, "weight", i), { 1,2,n_conv,n_embd },     backend_split);
                        layer.conv2_b = ml.create_tensor(ctx, tn(LLM_TENSOR_CONV2, "bias", i), {n_embd}, backend, false);
                        layer.lf_output_norm = ml.create_tensor(ctx, tn(LLM_TENSOR_LF_OUTPUT_NORM, "weight", i), {n_embd}, backend, false);
...
static void llm_build_conv_input_store(
    ...
    struct ggml_tensor* conv_input_cache_view = ggml_view_1d(ctx, kv.conv_inputs[il], n_tokens* n_embd_gqa,
        (ggml_row_size(conv_input->type, n_embd_gqa)) * kv_head);
...
struct llm_build_context {
    ...
    struct ggml_cgraph * build_llama() {
        ...
        // 使用get_rows代替切片
        //struct ggml_tensor* inp_tokens2 = ggml_new_tensor_1d(ctx0, GGML_TYPE_I32, batch.n_tokens);

        for (int il = 0; il < n_layer; ++il) {
            struct ggml_tensor * inpSA = inpL;

            // norm norm:[hid_size,1,1,1] inpl:[hid_size,seq_len,1,1]
            ...
            // self-attention
            {
                ...
                struct ggml_tensor* Vcur = ggml_mul_mat(ctx0, model.layers[il].wv, cur);//cur：[hid_size,seq_len,1,1]
                cb(Vcur, "Vcur", il);
                ...
                llm_build_conv_input_store(ctx0, hparams, kv_self, gf, cur2, n_ctx, n_tokens, kv_head, cb, il);
                if (kv_self.head > 0) {
                    struct ggml_tensor* cur2_before =ggml_view_2d(ctx0, kv_self.conv_inputs[il], kv_self.head, n_embd_gqa,ggml_element_size(kv_self.conv_inputs[il]) * kv_self.head,0);
                ...
                {
                    // 1,2,2048,1024
                    struct ggml_tensor* conv1_w = ggml_reshape_3d(ctx0, model.layers[il].conv1_w, 2, 2048, 1024);
                    //conv1
                    struct ggml_tensor* Conv_1 = ggml_conv_1d(ctx0, conv1_w, cur2, 1, 1, 1, kv_self.head);//[ow,oh,oc,n]
                    cb(Conv_1, "Conv_1", il);
                    ...
                    //conv2
                    struct ggml_tensor* conv2_w = ggml_reshape_4d(ctx0, model.layers[il].conv2_w, 2, 1024, 2048, 1);
                    struct ggml_tensor* Conv_2 = ggml_conv_1d(ctx0, conv2_w, Conv_1, 1, 1, 1, kv_self.head);
                    cb(Conv_2, "Conv_2", il);
...
static int llama_decode_internal(
    ...
    //ggml_graph_print(gf);
    //ggml_graph_export(gf, "e:/a.txt");
    //struct ggml_tensor* aa = gf->nodes[2];
    //printf("%s;  size%d x %d x %d x %d:\n", aa->name, aa->ne[0], aa->ne[1], aa->ne[2], aa->ne[3]);
    /*struct ggml_tensor* aa_list[10];
    for (int i = 0; i <= gf->n_nodes; i++) {
        struct ggml_tensor* aa = gf->nodes[i];
        printf("%d: %s;  size%d x %d x %d x %d:\n", i,aa->name,aa->ne[0], aa->ne[1], aa->ne[2], aa->ne[3] );
        if (i < 10) {
            aa_list[i] = aa;
        }
    }*/
    ...
    //       need to update the graphs to skip "result_output"
    {
        ...
        /*for (int i = 0; i < 2; i++) {
            std::vector<float> bb;
            int s1 = aa_list[i]->ne[0];
            int s2 = aa_list[i]->ne[1];
            bb.resize(s1 * s2);
            ggml_backend_tensor_get(res, bb.data(), (s1* s2 - 1) * sizeof(float), s1*s2 * sizeof(float));
            int a = 0;
        ...
        if (batch.logits) {
            ...
            //std::sort(logits_out.rbegin(), logits_out.rend());
