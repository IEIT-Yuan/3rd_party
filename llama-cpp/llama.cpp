...
static std::map<llm_arch, const char *> LLM_ARCH_NAMES = {
    ...
    { LLM_ARCH_YUAN2,           "yuan2"      },
    { LLM_ARCH_YUAN2_MOE,           "yuan2_moe"      },
...
enum llm_tensor {
    ...
    LLM_TENSOR_CONV1,
    LLM_TENSOR_CONV2,
    LLM_TENSOR_LF_OUTPUT_NORM,
    LLM_TENSOR_ATTN_K,
    LLM_TENSOR_ATTN_V,
    LLM_TENSOR_MLP_ATTN_Q,
    LLM_TENSOR_MLP_ATTN_K,
    LLM_TENSOR_MLP_ATTN_V,
    LLM_TENSOR_ATTN_QKV,
    LLM_TENSOR_ATTN_OUT,
...
static std::map<llm_arch, std::map<llm_tensor, std::string>> LLM_TENSOR_NAMES = {
    {
        LLM_ARCH_LLAMA,
        {
            ...
            { LLM_TENSOR_CONV1,          "blk.%d.conv1"},
            { LLM_TENSOR_CONV2, "blk.%d.conv2"},
            { LLM_TENSOR_LF_OUTPUT_NORM, "blk.%d.lf_output_norm"},
            { LLM_TENSOR_ATTN_Q,          "blk.%d.attn_q" },
            { LLM_TENSOR_ATTN_K,          "blk.%d.attn_k" },
            { LLM_TENSOR_ATTN_V,          "blk.%d.attn_v" },
            { LLM_TENSOR_MLP_ATTN_Q,          "blk.%d.mlp_attn_q" },
            { LLM_TENSOR_MLP_ATTN_K,          "blk.%d.mlp_attn_k" },
            { LLM_TENSOR_MLP_ATTN_V,          "blk.%d.mlp_attn_v" },
...
struct llama_layer {
    ...
    struct ggml_tensor* lf_output_norm;
    struct ggml_tensor * wq;
    struct ggml_tensor * wk;
    struct ggml_tensor * wv;
    struct ggml_tensor * wo;
    struct ggml_tensor * wqkv;
    struct ggml_tensor* mlp_attn_wq;
    struct ggml_tensor* mlp_attn_wk;
    struct ggml_tensor* mlp_attn_wv;
...
struct llama_kv_cache {
    ...
    std::vector<struct ggml_tensor*> conv_inputs;
...
static bool llama_kv_cache_init(
    ...
    for (auto & it : buft_layer_count) {
        ...
        struct ggml_init_params params = {
            /*.mem_size   =*/ 3u*n_layers*ggml_tensor_overhead(),
            /*.mem_buffer =*/ NULL,
            /*.no_alloc   =*/ true,
        };
    ...
    cache.conv_inputs.reserve(n_layer);
    ...
    for (int i = 0; i < (int) n_layer; i++) {
        ...
        if (hparams.n_expert_used > 0) {
            //conv_input = ggml_new_tensor_1d(ctx, GGML_TYPE_F32, hparams.n_embd * n_ctx);//保存最近的2个
            conv_input = ggml_new_tensor_1d(ctx, GGML_TYPE_F32, hparams.n_embd * 2);//保存最近的2个
        }
        else {
            //conv_input = ggml_new_tensor_1d(ctx, GGML_TYPE_F32, n_embd_v_gqa * n_ctx);//保存最近的2个
            conv_input = ggml_new_tensor_1d(ctx, GGML_TYPE_F32, n_embd_v_gqa * 2);//保存最近的2个
        }

        ggml_format_name(k, "cache_k_l%d", i);
        ggml_format_name(v, "cache_v_l%d", i);
        ggml_format_name(conv_input, "cache_conv_input%d", i);
        cache.k_l.push_back(k);
        cache.v_l.push_back(v);
        cache.conv_inputs.push_back(conv_input);
    }
...
static void llm_load_hparams(
    ...
    if (hparams.n_expert_used > 0) {
        hparams.n_embd_head_k = hparams.n_embd / hparams.n_head*2;
    ...
    switch (model.arch) {
        ...
        case LLM_ARCH_YUAN2_MOE:
        {
            ml.get_key(LLM_KV_ATTENTION_LAYERNORM_RMS_EPS, hparams.f_norm_rms_eps);

            switch (hparams.n_layer) {
            case 22: model.type = e_model::MODEL_1B; break;
            case 24: model.type = e_model::MODEL_40B; break;
            case 32: model.type = e_model::MODEL_7B; break;
            case 40: model.type = e_model::MODEL_13B; break;
            case 48: model.type = e_model::MODEL_34B; break;
            case 60: model.type = e_model::MODEL_30B; break;
            ...
            case LLM_ARCH_MINICPM:
                {
                    ...
                    for (int i = 0; i < n_layer; ++i) {
                        ...
                        if (hparams.n_expert_used > 0) {
                            layer.wq = ml.create_tensor(ctx_split, tn(LLM_TENSOR_ATTN_Q, "weight", i), { n_embd, n_embd * 2 });
                            layer.wo = ml.create_tensor(ctx_split, tn(LLM_TENSOR_ATTN_OUT, "weight", i), { n_embd * 2, n_embd });
                        }
                        else {
                        ...
                        layer.conv1_w = ml.create_tensor(ctx_split, tn(LLM_TENSOR_CONV1, "weight", i), { 1,2,n_embd,n_conv });
                        layer.conv1_b = ml.create_tensor(ctx_split, tn(LLM_TENSOR_CONV1, "bias", i), { n_conv }, false);
                        layer.conv2_w = ml.create_tensor(ctx_split, tn(LLM_TENSOR_CONV2, "weight", i), { 1,2,n_conv,n_embd } );
                        layer.conv2_b = ml.create_tensor(ctx_split, tn(LLM_TENSOR_CONV2, "bias", i), { n_embd },  false);
                        layer.lf_output_norm = ml.create_tensor(ctx_split, tn(LLM_TENSOR_LF_OUTPUT_NORM, "weight", i), { n_embd }, false);

                        layer.mlp_attn_wq = ml.create_tensor(ctx_split, tn(LLM_TENSOR_MLP_ATTN_Q, "weight", i), { n_embd,n_expert }, false);
                        layer.mlp_attn_wk = ml.create_tensor(ctx_split, tn(LLM_TENSOR_MLP_ATTN_K, "weight", i), { n_embd,n_expert }, false);
                        layer.mlp_attn_wv = ml.create_tensor(ctx_split, tn(LLM_TENSOR_MLP_ATTN_V, "weight", i), { n_embd,n_expert }, false);
                        ...
                        if (hparams.n_expert == 0) {
                            ...
                            for (uint32_t x = 0; x < hparams.n_expert; ++x) {
                                layer.ffn_gate_exp[x] = ml.create_tensor(ctx_split, tn(LLM_TENSOR_FFN_GATE_EXP, "weight", i, x), {n_embd,   n_ff*2});
...
static void llm_build_conv_input_store(
    struct ggml_context* ctx,
    const llama_hparams& hparams,
    const llama_kv_cache& kv,
    struct ggml_cgraph* graph,
    ...
    int64_t n_embd_gqa = 0;
    if (hparams.n_expert_used > 0) {
        n_embd_gqa = hparams.n_embd_gqa()/2;
    }
    else {
    ...
    struct ggml_tensor* conv_input_cache_view = ggml_view_1d(ctx, kv.conv_inputs[il], n_tokens * n_embd_gqa,
        (ggml_row_size(conv_input->type, n_embd_gqa)) * 0);
    cb(conv_input_cache_view, "conv_input_cache_view", il);

    ggml_build_forward_expand(graph, ggml_cpy(ctx, conv_input, conv_input_cache_view));
}
...
static struct ggml_tensor * llm_build_ffn(
        struct ggml_context * ctx,
         ...
         const llm_build_cb & cb,
                        int   il) {
    struct ggml_tensor* tmp_up = ggml_mul_mat(ctx, up, cur);
    cb(tmp_up, "ffn_up", il);

    if (up_b) {
        tmp_up = ggml_add(ctx, tmp_up, up_b);
        cb(tmp_up, "ffn_up_b", il);
    }

    struct ggml_tensor* cur_gate;
    if (gate) {
        switch (type_gate) {
        case LLM_FFN_SEQ:
        {
            cur = ggml_mul_mat(ctx, gate, tmp_up);
            cb(cur, "ffn_gate", il);
        } break;
        case LLM_FFN_PAR:
        {
            cur_gate = ggml_mul_mat(ctx, gate, cur);
            cb(cur, "ffn_gate", il);
        } break;
        }
    ...
    struct ggml_cgraph * build_llama() {
        ...
        for (int il = 0; il < n_layer; ++il) {
            ...
            // self-attention
            {
                ...
                //cur [2048,512,1,1] ->[512,2048,1,1]
                struct ggml_tensor* cur2 = ggml_reshape_2d(ctx0, cur, cur->ne[1], cur->ne[0]);
                int32_t n_tokens_save = 2;
                if (kv_self.head == 0)  // 第1个token
                    n_tokens_save = std::min(n_tokens, 2);

                int is_continue_gen = 0;
                //llm_build_conv_input_store(ctx0, hparams, kv_self, gf, cur2, n_ctx, 2, kv_head, cb, il);
                if (kv_self.head > 0) {
                    struct ggml_tensor* cur2_before;
                    //int32_t tokens_last = kv_self.head;
                    //int32_t n_tokens_load = std::min(tokens_last, 2);
                    if (hparams.n_expert_used > 0) {
                        //cur2_before = ggml_view_2d(ctx0, kv_self.conv_inputs[il], kv_self.head, n_embd_gqa/2, ggml_element_size(kv_self.conv_inputs[il]) * kv_self.head, 0);
                        cur2_before = ggml_view_2d(ctx0, kv_self.conv_inputs[il], 2, n_embd_gqa/2, ggml_element_size(kv_self.conv_inputs[il]) * 2, 0);
                    }
                    else {
                        //cur2_before = ggml_view_2d(ctx0, kv_self.conv_inputs[il], kv_self.head, n_embd_gqa, ggml_element_size(kv_self.conv_inputs[il]) * kv_self.head, 0);
                        cur2_before = ggml_view_2d(ctx0, kv_self.conv_inputs[il], 2, n_embd_gqa, ggml_element_size(kv_self.conv_inputs[il]) * 2, 0);
                    }
                    if( kv_self.head>0 &&cur2->ne[0]==1){
                        cur2 = ggml_concat(ctx0, cur2, cur2_before);
                        is_continue_gen = kv_self.head;
                    }

                }
                llm_build_conv_input_store(ctx0, hparams, kv_self, gf, cur2, n_ctx, n_tokens_save, kv_head, cb, il);
                //lfa
                {
                    // 1,2,2048,1024
                    struct ggml_tensor* conv1_w = ggml_reshape_3d(ctx0, model.layers[il].conv1_w, 2, hparams.n_embd, hparams.n_embd / 2);
                    //conv1
                    struct ggml_tensor* Conv_1 = ggml_conv_1d(ctx0, conv1_w, cur2, 1, 1, 1, is_continue_gen);//[ow,oh,oc,n]
                    cb(Conv_1, "Conv_1", il);


                    //cb(inp_tokens2, "inp_tokens", il);
                    //Conv_1 = ggml_get_rows(ctx0, Conv_1, inp_tokens2);
                    //Conv_1 = ggml_reshape_2d(ctx0, Conv_1, Conv_1->ne[1], Conv_1->ne[0]);
                    if (model.layers[il].conv1_b) {
                        struct ggml_tensor* conv1_b = model.layers[il].conv1_b;
                        conv1_b = ggml_reshape_4d(ctx0, conv1_b, conv1_b->ne[2], conv1_b->ne[0], conv1_b->ne[1], conv1_b->ne[3]);
                        conv1_b = ggml_repeat(ctx0, conv1_b, Conv_1);
                        Conv_1 = ggml_add(ctx0, Conv_1, conv1_b);
                        cb(Conv_1, "Conv_1", il);
                    }
                    //conv2
                    struct ggml_tensor* conv2_w = ggml_reshape_4d(ctx0, model.layers[il].conv2_w, 2, hparams.n_embd / 2, hparams.n_embd, 1);
                    struct ggml_tensor* Conv_2 = ggml_conv_1d(ctx0, conv2_w, Conv_1, 1, 1, 1, is_continue_gen);
                    cb(Conv_2, "Conv_2", il);

                    //Conv_2 = ggml_get_rows(ctx0, Conv_2, inp_tokens2);
                    //Conv_2 = ggml_reshape_2d(ctx0, Conv_2, Conv_2->ne[1], Conv_2->ne[0]);
                    if (model.layers[il].conv2_b) {
                        struct ggml_tensor* conv2_b = model.layers[il].conv2_b;
                        conv2_b = ggml_reshape_4d(ctx0, conv2_b, conv2_b->ne[2], conv2_b->ne[0], conv2_b->ne[1], conv2_b->ne[3]);
                        conv2_b = ggml_repeat(ctx0, conv2_b, Conv_2);
                        Conv_2 = ggml_add(ctx0, Conv_2, conv2_b);
                        Conv_2 = ggml_reshape_4d(ctx0, Conv_2, Conv_2->ne[1], Conv_2->ne[0], Conv_2->ne[2], Conv_2->ne[3]);
                        cb(Conv_2, "Conv_2", il);
                    }

                    //rms_norm
                    Conv_2 = ggml_add(ctx0, Conv_2, cur);
                    cur = llm_build_norm(ctx0, Conv_2, hparams, model.layers[il].lf_output_norm, NULL, LLM_NORM_RMS, cb, il);
                    cb(cur, "attn_norm", il);

                }

                struct ggml_tensor* Qcur_before_mix = ggml_mul_mat(ctx0, model.layers[il].wq, cur);
                if (hparams.n_expert_used > 0) {
                    cb(Qcur_before_mix, "Qcur_before_mix_moe", il);
                }
                else {
                    cb(Qcur_before_mix, "Qcur_before_mix", il);
                }

                /*if (model.layers[il].bq) {
                    Qcur_before_mix = ggml_add(ctx0, Qcur_before_mix, model.layers[il].bq);
                    cb(Qcur_before_mix, "Qcur_before_mix", il);
                }*/

                struct ggml_tensor* Kcur_before_mix = ggml_mul_mat(ctx0, model.layers[il].wk, cur);
                if (hparams.n_expert_used > 0) {
                    cb(Kcur_before_mix, "Kcur_before_mix_moe", il);
                }
                else {
                    cb(Kcur_before_mix, "Kcur_before_mix", il);
                }

                /*if (model.layers[il].bk) {
                    Kcur_before_mix = ggml_add(ctx0, Kcur_before_mix, model.layers[il].bk);
                    cb(Kcur_before_mix, "Kcur_before_mix", il);
                }*/


                //q、k混合
                struct ggml_tensor* Qcur = ggml_concat(ctx0, Qcur_before_mix, Kcur_before_mix);
                cb(Qcur, "Qcur", il);
                struct ggml_tensor* Kcur = ggml_concat(ctx0, Kcur_before_mix, Qcur_before_mix);
                cb(Kcur, "Kcur", il);

                if (hparams.n_expert_used > 0) {
                    Qcur = ggml_rope_custom(
                        ctx0, ggml_reshape_3d(ctx0, Qcur, n_embd_head, n_head, n_tokens), inp_pos,
                        n_embd_head/2, 0, 0, n_orig_ctx, freq_base, freq_scale,
                        ext_factor, attn_factor, beta_fast, beta_slow
                    );
                }
                else {
            ...
            if (hparams.n_expert == 0) {
                ...
                struct ggml_tensor* mlp_Qcur = cur;
                mlp_Qcur = ggml_mul_mat(ctx0, model.layers[il].mlp_attn_wq, mlp_Qcur);
                struct ggml_tensor* Q = ggml_reshape_3d(ctx0, mlp_Qcur, mlp_Qcur->ne[2], mlp_Qcur->ne[0], mlp_Qcur->ne[1]);
                struct ggml_tensor* mlp_Kcur = cur;

                mlp_Kcur = ggml_mul_mat(ctx0, model.layers[il].mlp_attn_wk, mlp_Kcur);
                struct ggml_tensor* K = ggml_reshape_3d(ctx0, mlp_Kcur, mlp_Kcur->ne[2], mlp_Kcur->ne[0], mlp_Kcur->ne[1]);

                struct ggml_tensor* mlp_Vcur = cur;
                mlp_Vcur = ggml_mul_mat(ctx0, model.layers[il].mlp_attn_wv, mlp_Vcur);
                struct ggml_tensor* V = ggml_reshape_3d(ctx0, mlp_Vcur, mlp_Vcur->ne[0], mlp_Vcur->ne[2], mlp_Vcur->ne[1]);
                ...
                struct ggml_tensor* logits = ggml_mul_mat(ctx0, V, KQ);
                logits = ggml_reshape_3d(ctx0, logits, logits->ne[1], logits->ne[2], logits->ne[0]);
                ...
                for (int i = 0; i < n_expert_used; ++i) {
                    ...
                    cur_expert = ggml_mul_mat_id(ctx0, model.layers[il].ffn_down_exp, n_expert, selected_experts, i, cur_gate); // [n_tokens, n_embd]
