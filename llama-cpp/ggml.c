...
inline static void ggml_vec_silu_f32_moe(const int n, float* y, const float* x) {
    uint16_t t;
    for (int i = 0; i < n; ++i) {
        ggml_fp16_t fp16 = GGML_FP32_TO_FP16(x[i]);
        float x2 = x[i + n];
        memcpy(&t, &fp16, sizeof(uint16_t));
        y[i] = GGML_FP16_TO_FP32(ggml_table_silu_f16[t]) * x2;
    }
}
#else
inline static void ggml_vec_silu_f32(const int n, float * y, const float * x) {
...
struct ggml_tensor * ggml_concat(
    ...
    if (memcmp(b->name, "cache_conv_input0(view)", 15) != 0) {
        GGML_ASSERT(a->ne[0] == b->ne[0] && a->ne[1] == b->ne[1] && a->ne[3] == b->ne[3]);
    }
    ...
    if (memcmp(a->name, "Qcur", 4) == 0 || memcmp(a->name, "Kcur", 4) == 0) {
        result = ggml_new_tensor_4d(ctx, a->type, a->ne[0], a->ne[1], a->ne[2], a->ne[3]);
    }
    else if (memcmp(b->name, "cache_conv_input0(view)", 15) == 0) {
        result = ggml_new_tensor_4d(ctx, b->type, a->ne[0] + b->ne[0], a->ne[1], a->ne[2], a->ne[3]);
    }
    else {
        result = ggml_new_tensor_4d(ctx, a->type, a->ne[0], a->ne[1], a->ne[2] + b->ne[2], a->ne[3]);
    }
...
struct ggml_tensor * ggml_get_rows(
    ...
    if (a->ne[1] == 1024 || a->ne[1] == 2048) {
        struct ggml_tensor* result = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, a->ne[1], a->ne[0] - 1);
        result->op = GGML_OP_GET_ROWS;
        result->grad = is_node ? ggml_dup_tensor(ctx, result) : NULL;
        result->src[0] = ggml_reshape_2d(ctx, a, a->ne[1], a->ne[0]);
        result->src[1] = b;
        return result;
    }
    else {
...
GGML_API struct ggml_tensor * ggml_conv_1d(
        ...
        int                   d0,
        int                   is_cache) {
    struct ggml_tensor * im2col = ggml_im2col(ctx, a, b, s0, 0, p0, 0, d0, 0, false, GGML_TYPE_F16, is_cache); // [N, OL, IC * K]
...
struct ggml_tensor* ggml_conv_1d_ph(
        ...
        int                   s,
        int                   d) {
    return ggml_conv_1d(ctx, a, b, s, a->ne[0] / 2, d,0);
}
...
struct ggml_tensor * ggml_conv_depthwise_2d(
    ...
    struct ggml_tensor * im2col = ggml_im2col(ctx, new_a,
                                        ...
                                        s0, s1, p0, p1, d0, d1, true, GGML_TYPE_F16,0); // [N * IC, OH, OW, KH * KW]
...
struct ggml_tensor * ggml_im2col(
    ...
    int64_t OW = ggml_calc_conv_output_size(b->ne[0], a->ne[0], s0, p0, d0) - 1;
    if (is_cache > 0 ) {
        //if (b->ne[0] <= 3) { //除第一个token之外，逐个token生成
        if (memcmp(ggml_get_name(b), "Conv_1", 6) == 0) { //conv2
            OW = 1;
        }
        else { //conv1
            OW = 2;
    ...
    int32_t params[] = { s0, s1, p0, p1, d0, d1, (is_2D ? 1 : 0),is_cache };
    ggml_set_op_params(result, params, sizeof(params));
...
struct ggml_tensor * ggml_conv_2d(
        ...
        int                  d0,
        int                  d1) {
    struct ggml_tensor * im2col = ggml_im2col(ctx, a, b, s0, s1, p0, p1, d0, d1, true, GGML_TYPE_F16,0); // [N, OH, OW, IC * KH * KW]
...
static struct ggml_tensor * ggml_unary_impl(
    ...
    struct ggml_tensor* result;
    if (memcmp(ggml_get_name(a), "ffn_moe_gate", 12) == 0) {
        result = ggml_new_tensor_2d(ctx, a->type, a->ne[0] / 2, a->ne[1]);
    }
    else {
        result = inplace ? ggml_view_tensor(ctx, a) : ggml_dup_tensor(ctx, a);
    }
...
// A simplified version of ggml_compute_forward_dup that doesn't do float upcasting, and just plain old memcpy.
static void ggml_compute_forward_dup_bytes2(
    const struct ggml_compute_params* params,
    struct ggml_tensor* dst) {
    ...
    if (ggml_is_contiguous(src0) && ggml_is_contiguous(dst)) {
        ...
        int dst_lap = 0;
        if (src0->ne[0] >= 2)
            src0_lap = src0->ne[0] - 2;
        else
            dst_lap = 1;
        //const float* x1 = (float*)((char*)src0->data);
        //printf("%f %f %f\n", x1[0], x1[1], x1[2]);

        /*if (dst_lap == 1) {//输入token==1 移位；
            memcpy(
                ((char*)dst->data + ie0 * nb0),
                ((char*)dst->data + ie0 * nb00 + dst_lap * src0->nb[0] * src0->ne[1]),
                src0->ne[1] * ggml_type_size(src0->type));
            const float* x3 = (float*)((char*)dst->data);
            printf("%f %f %f   %f %f %f\n", x3[0], x3[1], x3[2], x3[2048], x3[2049], x3[2050]);
            memcpy(
                ((char*)dst->data + ie0 * nb0 + dst_lap * src0->nb[0] * src0->ne[1]),
                ((char*)src0->data + ie0 * nb00 ),
                src0->ne[1] * ggml_type_size(src0->type));
            const float* x4 = (float*)((char*)dst->data);
            printf("%f %f %f   %f %f %f\n", x4[0], x4[1], x4[2], x4[2048], x4[2049], x4[2050]);
            printf("%f %f %f   \n", x1[0], x1[1], x1[2]);
            return;
        }*/

        if (ie0 < ie1) { //第一次 输入token>=2
            memcpy(
                ((char*)dst->data + ie0 * nb0 ),
                ((char*)src0->data + ie0 * nb00 + src0_lap*src0->nb[0] * src0->ne[1]),
                (ie1 - ie0) * ggml_type_size(src0->type));
        }
        /*const float* x2 = (float*)((char*)dst->data);
        printf("%f %f %f\n", x2[0], x2[1], x2[2]); */
        return;
    }
    else {
...
static void ggml_compute_forward_dup(
    ...
    const struct ggml_tensor * src0 = dst->src[0];
    if (memcmp(ggml_get_name(dst), "conv_input_cache_view", 20) == 0) {
        ggml_compute_forward_dup_bytes2(params, dst);
        return;
...
static void ggml_compute_forward_add_f32(
    ...
    //char* bb = "Conv_1";
    //int c = memcmp(ggml_get_name(gf_res->nodes[i]),bb,5);
    //lfa最后一步的norm，输入：conv2的输出，需要add lfa的输入；
    if (memcmp(ggml_get_name(src0), "Conv_2", 6) == 0 && memcmp(ggml_get_name(src1), "attn_norm-0", 10) == 0)
    {
        //int hid_size = src0->ne[0];
        int hid_size = ne00;
        //int seq_len = src0->ne[1];
        int seq_len = ne01;
        float* temp_ptr;
        temp_ptr = (float*)malloc(sizeof(float) * hid_size * seq_len);
        float* tmp_ptr2 = (float*)((char*)src0->data);
        //printf("before: %f %f %f %f\n", tmp_ptr2[0], tmp_ptr2[1], tmp_ptr2[2], tmp_ptr2[3]);
        for (int i = 0; i < hid_size; i++) {
            for (int j = 0; j < seq_len; j++) {
                temp_ptr[j * hid_size + i] = tmp_ptr2[i * seq_len + j];
                //printf("%s", "hh");
            }
        }
        //printf("after-tmp: %f %f %f %f\n", temp_ptr[0], temp_ptr[1], temp_ptr[2], temp_ptr[3]);
        memcpy(tmp_ptr2, temp_ptr, hid_size * seq_len * sizeof(float));
        //printf("memcpy: %f %f %f %f\n", tmp_ptr2[0], tmp_ptr2[1], tmp_ptr2[2], tmp_ptr2[3]);
        free(temp_ptr);
    }
...
static void ggml_compute_forward_concat_f32(
    ...
    int64_t head_dim = 0;
    int64_t n_heads = 0;
    if (memcmp(src0->name, "Qcur_before_mix_moe", 19) == 0 || memcmp(src0->name, "Kcur_before_mix_moe", 19) == 0) {
        //yuan2-moe
        head_dim = 256;
        n_heads = 16;
    }
    ...
    if (memcmp(src0->name, "Qcur", 4) == 0) {
        for (int i3 = 0; i3 < ne3; i3++) {
            for (int i2 = ith; i2 < ne2; i2 += nth) {
                for (int i1 = 0; i1 < ne1; i1++) {
                    for (int i0 = 0; i0 < n_heads / 2; i0++) {  //n_heads必须是2的倍数；循环第一个token的2048，32个64；q，v各得16个；
                        const float* x1 = (float*)((char*)src0->data + i0 * 2 * head_dim * nb00 + i1 * nb01 + i2 * nb02 + i3 * nb03);
                        float* y = (float*)((char*)dst->data + i0 * head_dim * nb0 + i1 * nb01 + i2 * nb02 + i3 * nb03);
                        memcpy(y, x1, head_dim * nb0);
                        //printf("%f %f %f %f\n", y[0], y[1], y[62], y[63]);
                    }
                    for (int i0 = n_heads / 2; i0 < n_heads; i0++) {  //循环第一个token的2048，32个64；q，v各得16个；
                        const float* x2 = (float*)((char*)src1->data + (i0 - n_heads / 2) * 2 * head_dim * nb00 + i1 * nb01 + i2 * nb02 + i3 * nb03);
                        float* y = (float*)((char*)dst->data + i0 * head_dim * nb0 + i1 * nb01 + i2 * nb02 + i3 * nb03);
                        memcpy(y, x2, head_dim * nb0);
                        //printf("%f %f %f %f\n", y[0], y[1], y[62], y[63]);
                    }
                }
    ...
    else if (memcmp(src0->name, "Kcur", 4) == 0) {
        for (int i3 = 0; i3 < ne3; i3++) {
            for (int i2 = ith; i2 < ne2; i2 += nth) {
                for (int i1 = 0; i1 < ne1; i1++) {
                    for (int i0 = 0; i0 < n_heads / 2; i0++) {  //循环第一个token的2048，32个64；q，v各得16个；
                        const float* x1 = (float*)((char*)src1->data + (i0 * 2 + 1) * head_dim * nb00 + i1 * nb01 + i2 * nb02 + i3 * nb03);//q
                        float* y = (float*)((char*)dst->data + i0 * head_dim * nb0 + i1 * nb01 + i2 * nb02 + i3 * nb03);
                        memcpy(y, x1, head_dim * nb0);
                        //printf("%f %f %f %f\n", x1[0], x1[1], x1[62], x1[63]);
                        //printf("%f %f %f %f\n", y[0], y[1], y[62], y[63]);
                    }
                    for (int i0 = n_heads / 2; i0 < n_heads; i0++) {  //循环第一个token的2048，32个64；q，v各得16个；
                        const float* x2 = (float*)((char*)src0->data + ((i0 - n_heads / 2) * 2 + 1) * head_dim * nb00 + i1 * nb01 + i2 * nb02 + i3 * nb03);
                        float* y = (float*)((char*)dst->data + i0 * head_dim * nb0 + i1 * nb01 + i2 * nb02 + i3 * nb03);
                        memcpy(y, x2, head_dim * nb0);
                        //printf("%f %f %f %f\n", y[0], y[1], y[62], y[63]);
                    }
    ...
    else if (memcmp(src1->name, "cache_conv_input", 16) == 0) {
        const float* x0 = (float*)((char*)src0->data);
        const float* x1 = (float*)((char*)src1->data);

        float* y = (float*)((char*)dst->data);
        //*y = *x1;
        memcpy(y, x1, ne10 * ne11 * sizeof(float));
        memcpy(y + ne10 * ne11, x0, ne01 * sizeof(float));
        //printf("\n%f %f %f %f %f %f %f %f \n", x1[0], x1[2048], x0[0], y[0], y[2048], y[4095], y[4096], y[4097]);
    }
    else {
        for (int i3 = 0; i3 < ne3; i3++) {
            for (int i2 = ith; i2 < ne2; i2 += nth) {
...
static void ggml_compute_forward_silu_f32(
    ...
    for (int i1 = ir0; i1 < ir1; i1++) {
        ...
        if (memcmp(ggml_get_name(src0), "ffn_moe_gate", 12) == 0) {
            ggml_vec_silu_f32_moe(nc / 2,
                (float*)((char*)dst->data + i1 * (dst->nb[1])),
                (float*)((char*)src0->data + i1 * (src0->nb[1])));
        }
        else {
...
static void ggml_compute_forward_im2col_f16(
    ...
    const int32_t d1 = ((const int32_t *)(dst->op_params))[5];
    const bool is_2D = ((const int32_t *)(dst->op_params))[6] == 1;
    const int32_t is_cache = ((const int32_t*)(dst->op_params))[7];
    ...
    int64_t ow_end = OW;
    if (is_cache > 0) {
        if (memcmp(ggml_get_name(src1), "Conv_1", 6) == 0) {//conv2 ow=1
            ow_start = ne10 > OW ? ne10 - OW : 0;
        }
        else { //conv1 ow=2
            ow_start = ne10 > OW ? ne10 - OW : 0;
        }
    ...
    // im2col: [N, IC, IH, IW] => [N, OH, OW, IC*KH*KW]
    {
        ...
        for (int64_t in = 0; in < N; in++) {
            for (int64_t ioh = 0; ioh < OH; ioh++) { // 1
                for (int64_t iow = ow_start; iow < ow_end; iow++) {
                    for (int64_t iic = ith; iic < IC; iic += nth) {

                        // micro kernel
                        ggml_fp16_t* dst_data = wdata + (in * OH * OW + ioh * OW + iow - ow_start) * (IC * KH * KW); // [IC, KH, KW]
                        //const float * const src_data = (float *)((char *) src1->data + in*ofs0 + iic*ofs1); // [IH, IW]

                        //char * bb ="Conv_1";
                        //int c = memcmp(ggml_get_name(gf_res->nodes[i]),bb,5);
                        //printf("%d  %s\n", c, ggml_get_name(gf_res->nodes[i]));
                        if (memcmp(ggml_get_name(src1), "Conv_1", 6) == 0) { //contain conv2处理 "Conv_1-0"
                            const float* const src_data = (float*)((char*)src1->data + in * ofs0 + iic * ofs1); // [IH, IW]
                            for (int64_t ikh = 0; ikh < KH; ikh++) {  // 1
                                for (int64_t ikw = 0; ikw < KW; ikw++) {
                                    const int64_t iiw = iow * s0 + ikw * d0 - p0;
                                    const int64_t iih = ioh * s1 + ikh * d1 - p1;
                                    //printf("src:%f, src1:%lld,  src2:%lld\n", src_data[iih * IW + iiw ], in * ofs0 + iic * ofs1, iih * IW + iiw);
                                    //float a = src_data[iih * IW + iiw];
                                    if (iih < 0 || iih >= IH || iiw < 0 || iiw >= IW) {
                                        dst_data[iic * (KH * KW) + ikh * KW + ikw] = 0;
                                    }
                                    else {
                            ...
                            const float* const src_data = (float*)((char*)src1->data + in * ofs0 + iic * 4); // [IH, IW]
                            for (int64_t ikh = 0; ikh < KH; ikh++) {  // 1
                                for (int64_t ikw = 0; ikw < KW; ikw++) {
                                    const int64_t iiw = iow * s0 + ikw * d0 - p0;
                                    const int64_t iih = ioh * s1 + ikh * d1 - p1;
                                    if (iih < 0 || iih >= IH || iiw < 0 || iiw >= IW) {
                                        ...
                                        dst_data[iic * (KH * KW) + ikh * KW + ikw] = GGML_FP32_TO_FP16(src_data[iih * IW + iiw * IC]);
