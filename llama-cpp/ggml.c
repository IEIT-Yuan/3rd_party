...
static void ggml_vec_dot_f16(const int n, float * restrict s, ggml_fp16_t * restrict x, ggml_fp16_t * restrict y) {
    ...
    // leftovers
    for (int i = np; i < n; ++i) {
        //printf("%f  %f\n", GGML_FP16_TO_FP32(x[i]), GGML_FP16_TO_FP32(y[i]));
...
GGML_API struct ggml_tensor * ggml_conv_1d(
        ...
        int                   d0,
        int                   is_cache) {
    struct ggml_tensor * im2col = ggml_im2col(ctx, a, b, s0, 0, p0, 0, d0, 0, false, is_cache); // [N, OL, IC * K]
...
struct ggml_tensor * ggml_conv_2d(
        ...
        int                  d0,
        int                  d1) {
    struct ggml_tensor * im2col = ggml_im2col(ctx, a, b, s0, s1, p0, p1, d0, d1, true,0); // [N, OH, OW, IC * KH * KW]
...
static void ggml_compute_forward_mul_mat(
    ...
    //printf("%f %f %f %f %f  %f\n", dst_data[0], dst_data[1], dst_data[2], dst_data[2048], dst_data[2049], dst_data[2050]);
}
